...
Si preferís pasar el tema directamente, sin escribirlo a mano:

```bash
echo "Energías renovables" | docker run -i --rm lp-etapa3
```

Si no hay documentos para el tema consultado, el programa avisa con un mensaje y
termina.

---

## Alternativa sin Docker (opcional)

Solo si ya tenés instalados **sbt** y **Python 3 con spaCy**. Una vez:
...
sbt run            # ejecutar (pide el tema por teclado)
```
